...
test('[J-075] empty home shows the "Tap here..." secondary hint', async ({ page, request }) => {
  ...
  // Home.tsx — the empty CTA carries a 2-line message: the caps label
...
test('[J-076] tapping the empty-state pill opens AddPicker', async ({ page, request }) => {
  ...
  // Home.tsx wraps the entire empty CTA in a single <button onClick={onAddEntry}>.
  // App.tsx onAddEntry sets modal=add-picker — so tapping anywhere on the pill
...
test('[J-009] AddPicker search with no matches shows the empty-results block', async ({ page, request }) => {
  ...
  // AddPicker.tsx — emptySearch branch. Both the typed query and the
  ...
  // (AddPicker.tsx) which is ONLY rendered alongside list rows. Asserting
...
test('[J-077] AddPicker idle on a brand-new user shows "library is empty"; no Recent/All headers', async ({ page, request }) => {
  ...
  // AddPicker.tsx — idleEmpty fires when /products/recent AND
  ...
  // "Recent" / "All" section headers (AddPicker.tsx) only render when
  ...
  // only forward paths (AddPicker.tsx). Without them the empty-state
  // user is stuck.
...
test('[J-078] empty-day MacroSummary shows full default kcal goal as remaining + 0 consumed', async ({ page, request }) => {
  ...
  // MacroSummary.tsx sets kcalLeft=2400 and kcalOver=false. The empty day
  // shows "Kcal remaining" with no "Over budget" label.
  ...
  // spans (MacroSummary.tsx). Exact-match catches the standalone
  ...
  // Consumed column shows "0 kcal" (MacroSummary.tsx). Outer span's
