...
// branch (statements.ts) and can be adopted into the caller's library
// via POST /products/adopt/:id (products.ts). Non-barcoded products
...
async function seedAsUserA(
  ...
  const res = await request.post('/products', {
    ...
    // isProductBaseBody (products.ts) requires `brand` to be null
...
test('[J-034] barcoded product adopts silently on tap from global search', async ({
  ...
  // Flip AddPicker scope to Global (AddPicker.tsx) so the cross-user
  // search branch kicks in — without it, the request lacks ?global=1 and
  // user B never sees user A's catalog (statements.ts gated path).
  await page.getByRole('button', { name: 'Global', exact: true }).tap();
  await page.getByPlaceholder('Search products...').fill(name);

  // The result carries is_mine=false; App.tsx auto-POSTs to
...
test('[J-035] adopt is idempotent on (created_by, barcode)', async ({ page, request }) => {
  ...
  // first inserts a row (201, products.ts); the second hits the existing
  // fast path (products.ts) and returns 200 with the same row.
...
test('[J-036] non-barcoded products never cross users (privacy invariant, UI search)', async ({
  ...
  // (statements.ts) only surfaces cross-user rows where barcode IS NOT
  ...
  // AddPicker.tsx renders the empty-results block when the server
  ...
  // to button-role to skip the empty-state echo (`AddPicker.tsx`
...
test('[J-048] adopt rejects non-barcoded source with 400 not_adoptable', async ({
  ...
  // Privacy guard at products.ts. Even if a malicious user knew the
...
test('[J-049] adopt rejects unknown id with 404 not_found', async ({ page, request }) => {
  // products.ts: byIdAnyUser returns undefined for an id that does
...
test('[J-051] self-adopt fast path returns the same row, no duplicate insert', async ({
  ...
  // products.ts: when source.created_by === req.userId, the route
